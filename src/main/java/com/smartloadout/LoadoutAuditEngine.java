...
public class LoadoutAuditEngine
{
	...
	private AuditResult evaluate(ActivityTemplate template, LoadoutRule rule, LoadoutSnapshot snapshot)
	{
		...
		switch (rule.getType())
		{
			...
			case AMMO_OR_RANGED_WEAPON:
				return equippedGroup(
					template,
					rule,
					snapshot,
					"Ranged equipment present",
					"No matching ammo or ranged weapon equipped");
			case ATTACK_STYLE:
	...
	private AuditResult requiredItem(ActivityTemplate template, LoadoutRule rule, LoadoutSnapshot snapshot)
	{
		...
		return new AuditResult(
			present ? AuditSeverity.PASS : AuditSeverity.FAIL,
			rule.getTitle(),
			present ? "Required item is present." : "Required item is missing.");
	}
	...
	private AuditResult minimumQuantity(ActivityTemplate template, LoadoutRule rule, LoadoutSnapshot snapshot)
	{
		...
		return new AuditResult(
			enough ? AuditSeverity.PASS : AuditSeverity.FAIL,
			rule.getTitle(),
			enough ? "Quantity requirement met." : "Only found " + count + " of " + rule.getMinimumQuantity() + " required.");
	}
	...
	private AuditResult runePouchContains(ActivityTemplate template, LoadoutRule rule, LoadoutSnapshot snapshot)
	{
		...
		return new AuditResult(
			enough ? AuditSeverity.PASS : AuditSeverity.FAIL,
			rule.getTitle(),
			enough ? "Rune pouch requirement met." : "Rune pouch is missing required rune.");
	}

	private AuditResult equippedGroup(
	...
	{
		if (rule.getExpectedValue().trim().isEmpty())
		{
		...
		return new AuditResult(
			matches ? AuditSeverity.PASS : AuditSeverity.FAIL,
			rule.getTitle(),
			matches ? pass : fail + ": expected " + rule.getExpectedValue() + ".");
	}
	...
	private AuditResult wildernessTeleportLimit(ActivityTemplate template, LoadoutRule rule, LoadoutSnapshot snapshot)
	{
		...
		return new AuditResult(
			usable ? AuditSeverity.PASS : AuditSeverity.FAIL,
			rule.getTitle(),
			usable
				? "Teleport is usable at this Wilderness level."
				: "Teleport is blocked above level " + rule.getWildernessLimit() + " Wilderness.");
	}
	...
	private AuditResult duplicateSupplyWarning(ActivityTemplate template, LoadoutRule rule, LoadoutSnapshot snapshot)
	{
		...
		return new AuditResult(
			duplicate ? AuditSeverity.WARNING : AuditSeverity.PASS,
			rule.getTitle(),
			duplicate ? "Multiple supply variants from this group are present." : "No duplicate supply variants found.");
	}
